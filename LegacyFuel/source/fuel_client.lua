...
AddEventHandler('fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			ESX.ShowFloatingHelpNotification("Presiona ~r~[E]~w~ para cancelar.", vector3(stringCoords.x, stringCoords.y, stringCoords.z+0.8))
	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
				...
				ESX.ShowFloatingHelpNotification("Sal del vehículo para echar gasolina.", vector3(pumpCoords.x, pumpCoords.y, pumpCoords.z+0.8))
			else
					...
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								ESX.ShowFloatingHelpNotification("Presiona ~r~[E]~w~ para echar gasolina.", vector3(stringCoords.x, stringCoords.y, stringCoords.z+0.8))
								...
								ESX.ShowFloatingHelpNotification("~r~NO TIENES SUFICIENTE DINERO.", vector3(stringCoords.x, stringCoords.y, stringCoords.z+0.8))
							end
						elseif not canFuel then
							ESX.ShowFloatingHelpNotification("Presiona ~r~[E]~w~ para rellenar el bidón de gasolina.", vector3(stringCoords.x, stringCoords.y, stringCoords.z))
						else
							ESX.ShowFloatingHelpNotification("Tanque lleno.", vector3(stringCoords.x, stringCoords.y, stringCoords.z+0.8))
						end
					end
				elseif isNearPump then
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847) then
							ESX.ShowFloatingHelpNotification("Presiona ~r~[E]~w~ para comprar un bidón de gasolina.", vector3(stringCoords.x, stringCoords.y, stringCoords.z+0.8))
